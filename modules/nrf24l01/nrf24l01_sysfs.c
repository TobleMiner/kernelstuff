...
ssize_t nrf24l01_sysfs_show_channel(struct device* dev, struct device_attribute* attr, char* buf)
{
	...
	int err;
	if((err = nrf24l01_get_channel(NRF24L01_NRFCHR_TO_NRF((struct nrf24l01_chrdev*)dev_get_drvdata(dev)), &channel)))
		return err;
...
ssize_t nrf24l01_sysfs_store_channel(struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
	...
	if((err = kstrtouint(buff, 10, &channel)))
		goto exit_buffalloc;
	err = nrf24l01_set_channel(NRF24L01_NRFCHR_TO_NRF((struct nrf24l01_chrdev*)dev_get_drvdata(dev)), channel);
	if(err)
		goto exit_buffalloc;
	return count;
exit_buffalloc:
...
static ssize_t nrf24l01_sysfs_show_address(struct device* dev, char* buf, unsigned int pipe)
{
	...
	u64 addr = 0;
	char fmt[10];
	nrf24l01_t* nrf = NRF24L01_NRFCHR_TO_NRF((struct nrf24l01_chrdev*)dev_get_drvdata(dev));
...
static ssize_t nrf24l01_sysfs_store_enable(struct device* dev, const char* buf, size_t count, unsigned int pipe)
{
	...
	if((err = kstrtouint(str, 10, &state)))
		goto exit_stralloc;
	if((err = nrf24l01_set_pipe_enable(nrf, pipe, !!state)))
		goto exit_stralloc;
	err = count;
exit_stralloc:
	vfree(str);
